...
onready var obstacles = get_empty_cells()
...
func astar_add_walkable_cells(obstacles = []):
	...
	for y in range(map_size.y):
		for x in range(map_size.x):
			...
			#ТОЛСТОСТЬ Я НЕ ЗАДАЛ
			var weight = tile_set.tile_get_z_index(get_cellv(point))*3
...
func _draw():
	...
	for index in range(1, len(_point_path)):
		...
		draw_circle(current_point, BASE_LINE_WIDTH * 2.1, DRAW_COLOR)
		current_point = Vector2(current_point.x, current_point.y-1)
